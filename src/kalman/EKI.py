...
def EKI(u0, U, y, G, eta, Ntmax, IGamma):
    ...
    print(f"EKI-Non converge in {Ntmax} iterazioni. Misfit - norm(eta)2 = {teta - norm(eta) ** 2 :.6f}. Misfit / norm(eta)2 = {teta / norm(eta) ** 2 :.6f}")
    return u, TETA, res
...
def G(u, d, K, dx, dy, Gg, calc): #G lineare
    ...
    if Gg == 0:
        ...
        A = 1/(dx*dy) * (2*np.eye(K) -np.diag(np.ones(K-1),1) -np.diag(np.ones(K-1),-1)) +np.eye(K)
        g = np.linalg.inv(A)
        if calc:
        ...
        A = 1/(dx*dy) * (-2*np.diag(np.ones(K))+np.diag(np.ones(K-1),1)+np.diag(np.ones(K-1),-1))
        g = np.linalg.inv(A)
        if calc:
            gC0 = np.linalg.inv(A - np.eye(d))
            return gC0
        ...
        if calc: #Calcolo gC0
            A = np.exp(u)/dx**2 * (-np.diag(u[:-1], -1) + np.diag(u+np.append(u[1:],2*u[-1]-u[-2])) + np.diag(-u[1:], 1))
            gC0 = np.linalg.inv(A - np.eye(d))
            return gC0
        #se calc == False
        if len(np.shape(u)) == 1: # u è un vettore d. Serve per calcolare y e teta
            ...
            ud = u.flatten(); ud[d-1::d] = 0 # u[i-1] sottodiagonale
            uu = u.flatten(); uu[0::d] = 0 # u[i+1] sopradiagonale
            diagonals = [
                -ud[:-1]*np.exp(ud[:-1])/dx**2,  # subdiagonal
                (uc + np.hstack((u[:, 1:], 2 * u[:, -1:] - u[:, -2:-1])).flatten())*np.exp(uc)/dx**2,  # main diagonal
        ...
        if calc:
            A = 1/dx**2 * (2*np.eye(K) - np.diag(np.ones(K-1), 1) - np.diag(np.ones(K-1), -1))
        ...
        if calc:
            A = 1/dx**2 * (np.diag(u[:-1], -1) - 2*np.diag(u + np.append(u[1:], 2*u[-1]-u[-2])) + np.diag(2*u[1:]-u[:-1], 1))
            A += 1/dx * (-np.diag(np.log(np.abs(u+1e-8))) + np.diag(np.log(np.abs(u[:-1]+1e-8)), 1))
        ...
        if len(np.shape(u)) == 1: # u è un vettore d. Serve per calcolare y=g(u)+eta
            ...
            diagonals = [
                ...
                1/dx**2 * (-2*(uc + np.hstack((u[:, 1:], 2 * u[:, -1:] - u[:, -2:-1])).flatten())) + 1/dx * (-np.log(np.abs(uc+1e-8))) + (1/np.abs(uc+1e-8) + 1),  # main diagonal
                1/dx**2 * (2*uu[1:] - uc[:-1]) + 1/dx * (np.log(np.abs(uc[:-1]+1e-8))) # superdiagonal
            ]
            A = diags(diagonals, offsets=[-1, 0, 1], shape=(N * K, N * K))
            b = np.ones(N*K)
            p = spsolve(A, b)
        ...
        if len(np.shape(u)) == 1: # u è un vettore d. Serve per calcolare y e teta
            ...
            ud1 = ep.repeat(u1, "n -> (n rep)", rep=K); ud1[K-1::K] = 0 # u[i-1] sottodiagonale
            uu1 = ep.repeat(u1, "n -> (n rep)", rep=K); uu1[K-1::K] = 0 # u[i+1] sopradiagonale
            diagonals = [
            ...
            b = np.ones(N*K)
            #Condizioni al bordo
            A.data[1, 0::K] = 1; A.data[1, K-1::K] = 1
            b[0::K] = 0; b[K-1::K] = u2
            p = spsolve(A, b)
            return p.reshape((N, K))
...
def grafico(x, y, u0, uM, U, GU, GuM, teta, res, SALVA=False, savepath='\Desktop\\'):
    ...
    plt.xlabel('x')
    plt.legend()
    if SALVA: plt.savefig(savepath+'Dati Ricostruiti.png')
    ...
    plt.xlabel('x')
    plt.legend()
    if SALVA: plt.savefig(savepath+'Ricostruzione Controllo.png')
    ...
    plt.legend()
    if SALVA: plt.savefig(savepath+'Misfit.png')
    ...
    plt.xlabel('Iterazioni')
    plt.legend()
    if SALVA: plt.savefig(savepath+'Residuo.png')
    ...
    if SALVA: plt.savefig(savepath+'Controlli iniziali.png')

    plt.show()
    
def grafico2D(x, y, u0, uM, u, U, GU, GuM, teta, res, SALVA=False, savepath='\Desktop\\'):
    plt.figure(1)
    plt.title('Dati')
    plt.plot(x, y, 'x--y', label = 'osservazioni')
    plt.plot(x, GU, '-g', label = f'Sol Esatta')
    ...
    xi, yi = np.meshgrid(np.linspace(u0[:, 0].min(), u0[:, 0].max(), 100), 

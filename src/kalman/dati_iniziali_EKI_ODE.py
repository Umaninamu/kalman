...
def controllo_iniziale(y, d, K, N, dt, A, b, c, f, t):
    contr = 2
    if contr == 1:
        '''
        ...
        '''
        u0 = np.array([[y+0.25+np.random.randn() for _ in range(N)] for _ in range(d)])
    elif contr == 2:
        '''
        ...
        Funziona male per case == 12: se T grande (50) non vede i massimi
        '''
        u0 = np.random.normal(loc=y, scale=dt, size=(d, N, K))
    elif contr == 3:
        '''
        ...
        '''
        u0 = np.random.normal(loc=y, scale=dt*np.max(np.diff(c))*1.5, size=(d, N, K))
    elif contr == 4:
        '''
        ...
        '''
        u0 = np.random.normal(loc=odei.RungeKuttaNewton(f, y, t, dt, A, b, c), scale=1, size=(d, N, K))
    elif contr == 5:
        '''
        ...
        u0 = np.random.normal(loc=y_next, scale=1, size=(d, N, K))
